...
class ShortcodeValidator
{
    ...
    /**
     * Format array data for debug output without using print_r
     */
    public static function format_debug_data($data, $indent = 0): string
    {
        $output = '';
        ...
        if (is_array($data)) {
            ...
            foreach ($data as $key => $value) {
                $output .= $spaces . '  [' . $key . '] => ';
                if (is_array($value)) {
                    $output .= self::format_debug_data($value, $indent + 1);
                } else {
            ...
            $output .= is_bool($data) ? ($data ? 'true' : 'false') : $data;
        }
...
if (defined('WP_DEBUG') && WP_DEBUG) {
    add_action('wp_footer', function () {
        if (current_user_can('manage_options')) {
            ...
            echo '<pre>' . esc_html(ShortcodeValidator::format_debug_data($assets)) . '</pre>';
