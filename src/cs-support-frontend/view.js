...
class CSSupportFrontendEnhancer {
	...
	setupCustomColors() {
		const { successColor, warningColor, errorColor, primaryColor } =
		...
		if ( primaryColor ) {
			...
			this.container.style.setProperty(
				'--cs-primary-color-hover',
				this.darkenColor( primaryColor, 10 )
			);
		}
		if ( successColor ) {
			this.container.style.setProperty(
			...
			this.container.style.setProperty(
				'--cs-success-color-rgb',
				this.hexToRgb( successColor )
			);
		}
		if ( warningColor ) {
			this.container.style.setProperty(
			...
			this.container.style.setProperty(
				'--cs-warning-color-rgb',
				this.hexToRgb( warningColor )
			);
		}
		if ( errorColor ) {
	...
	createFilterButtons() {
		...
		const filters = [
			...
			{
				label: __( 'In Progress', 'clientsync-support' ),
				value: 'in-progress',
			},
			{
				label: __( 'Resolved', 'clientsync-support' ),
				value: 'resolved',
			},
		];
	...
	handleSearch( query ) {
		...
		this.container.dispatchEvent(
			new CustomEvent( 'csSearchTickets', {
				detail: { query },
			} )
		);
	}
	...
	handleFilter( filterValue, button ) {
		...
		this.container.dispatchEvent(
			new CustomEvent( 'csFilterTickets', {
	...
	handleSort( columnIndex, header ) {
		...
		// Clear other headers
		this.container
			.querySelectorAll( '.cs-support-tickets-table th' )
			.forEach( ( h ) => {
...
const getApiSettings = () => {
	...
	return {
		nonce:
			document
				.querySelector( 'meta[name="wp-rest-nonce"]' )
...
document.addEventListener( 'DOMContentLoaded', () => {
	...
	function showError( message ) {
		...
		const errorContainer =
			errorElement.querySelector( '.cs-support-error' );
	...
	function renderTicketsList() {
		content.innerHTML = '';
		const ticketsElement = document.importNode(
			ticketsTemplate.content,
			true
		);
