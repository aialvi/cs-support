...
export default function DebugCapabilities() {
	...
	const fetchDebugInfo = async () => {
		try {
			const response = await fetch("/wp-json/cs-support/v1/debug/user-capabilities", {
				headers: {
			...
			<div className="bg-white shadow rounded-lg">
				<div className="px-6 py-4 border-b border-gray-200">
					...
					<p className="text-sm text-gray-500">
						Debug information for current user's permissions and roles
					</p>
				</div>
				...
				<div className="p-6">
					{debugInfo ? (
						<div className="space-y-4">
							...
							<div>
								<h4 className="font-medium text-gray-900">Support Capabilities</h4>
								<div className="mt-2 space-y-2">
									{Object.entries(debugInfo.capabilities).map(([capability, hasCapability]) => (
							...
							<div className="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded">
								<h4 className="font-medium text-yellow-800">Permission Issues?</h4>
								<p className="mt-1 text-sm text-yellow-700">
									If you're seeing "Sorry, you are not allowed to access this page" errors:
								</p>
								<ul className="mt-2 text-sm text-yellow-700 list-disc list-inside">
									<li>Ensure you have the "view_all_tickets" or "edit_tickets" capability</li>
									<li>Check if you're assigned as "Support Agent" or "Support Manager" role</li>
									<li>Administrators should have "manage_options" capability</li>
								</ul>
							</div>
						</div>
					) : (
