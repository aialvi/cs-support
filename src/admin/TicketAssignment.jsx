...
export default function TicketAssignment({ ticket, onAssignmentChange }) {
	...
	const fetchTeamMembers = async () => {
		setLoading(true);
		try {
			const response = await fetch("/wp-json/cs-support/v1/team-members", {
				headers: {
	...
	const handleAssignment = async (assigneeId) => {
		...
		if (ticket.assignee_id && ticket.assignee_id != assigneeId) {
			const currentAssigneeName = currentAssignee?.display_name || 'Unknown';
			const newAssigneeName = assigneeId
				? teamMembers.find(m => m.ID == assigneeId)?.display_name || 'Unknown'
				: 'Unassigned';
						...
						<div className="py-1 max-h-60 overflow-y-auto z-50">
							...
							{teamMembers.map((member) => (
								<button
									...
									className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-100 flex items-center space-x-3 ${member.ID == ticket.assignee_id ? "bg-blue-50" : ""
										}`}
								>
									<img
									...
									{member.ID == ticket.assignee_id && (
										<div className="text-blue-600 text-xs">Current</div>
									)}
								</button>
							))}
						</div>
