...
export default function Settings() {
			...
			try {
				const [ settingsResponse, gdprResponse ] = await Promise.all( [
					fetch( CS_SUPPORT_HELPDESK_SETTINGS_CONFIG.apiUrl, {
						...
						headers: {
							'X-WP-Nonce':
								CS_SUPPORT_HELPDESK_SETTINGS_CONFIG.nonce,
						},
					} ),
					fetch(
						`${ CS_SUPPORT_HELPDESK_SETTINGS_CONFIG.apiBaseUrl }/gdpr/data-retention`,
						{
							method: 'GET',
							headers: {
								'X-WP-Nonce':
	...
	const handleSubmit = async ( e ) => {
		...
		try {
			...
			const generalSettingsResponse = await fetch(
				CS_SUPPORT_HELPDESK_SETTINGS_CONFIG.apiUrl,
				{
					method: 'POST',
					headers: {
			...
			// Save GDPR settings
			const gdprSettingsResponse = await fetch(
				`${ CS_SUPPORT_HELPDESK_SETTINGS_CONFIG.apiBaseUrl }/gdpr/data-retention`,
				{
					method: 'POST',
					headers: {
	...
	const handleDeleteData = async ( type ) => {
		...
		try {
			...
			if ( data.success ) {
				setGdprMessage(
					`✅ Your data has been ${
						type === 'delete' ? 'deleted' : 'anonymized'
					} successfully!`
				);
	...
	const handleRunCleanup = async () => {
		...
		try {
			...
			if ( data.success ) {
				setGdprMessage(
					`✅ Cleanup completed: ${ data.stats.tickets_deleted } tickets and ${ data.stats.replies_deleted } replies removed.`
				);
			} else {
					...
					<div className="bg-white shadow rounded-lg p-6">
						...
						<div className="space-y-4">
							<div>
								...
								<select
									value={
										settings.general.defaultPriority ||
										'normal'
									}
									onChange={ ( e ) =>
										handleChange(
								...
								<p className="mt-1 text-sm text-gray-500">
									This will be the default priority for all
									new tickets
								</p>
							</div>
							...
							<div>
								...
								<p className="mt-1 text-sm text-gray-500">
									Select a page where users will be redirected
									after creating a ticket. This page should
									contain the cs-support-frontend block to
									display their tickets.
								</p>
							</div>
					...
					<div className="bg-white shadow rounded-lg p-6">
						...
						<div className="space-y-4">
							...
							<div>
								...
								<select
									value={
										settings.ai?.model || 'gpt-3.5-turbo'
									}
									onChange={ ( e ) =>
										handleChange(
											'ai',
								...
								<div className="space-y-4">
									<div className="flex items-center">
										<input
											...
											checked={
												settings.gdpr.dataRetention
													.enabled
											}
											onChange={ ( e ) =>
												handleGdprSettingsChange(
													'enabled',
													e.target.checked
												)
											}
								...
								<div className="space-y-4">
									<div className="bg-blue-50 p-4 rounded-lg">
										<p className="text-sm text-blue-800 mb-3">
											...
											You have the right to export or
								...
								<div className="space-y-3 mb-6">
									<label className="flex items-center">
										...
										<span className="ml-2 text-sm">
											<strong>Anonymize</strong> - Remove
											personal identifiers but keep ticket
											content for support quality
										</span>
									</label>
									...
									<label className="flex items-center">
										...
										<span className="ml-2 text-sm">
											...
											Permanently remove all your tickets
											and data
										</span>
									</label>
								</div>
